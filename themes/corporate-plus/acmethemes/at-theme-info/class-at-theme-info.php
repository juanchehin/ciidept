...
if ( ! class_exists( 'Corporate_Plus_Theme_Info' ) ) {
    class Corporate_Plus_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__('Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.','corporate-plus'), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '" class="button text-decoration-none">' . sprintf( __('Get started with %s','corporate-plus'), $this->theme_name ) . '</a></p>' );
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'corporate-plus-info-js', 'at_theme_info_object', array(
